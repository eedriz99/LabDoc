...
- Topology links have an optional `style` (`access`/`trunk`); colors/widths live in `topoLinkStyle` (Go) and `LINK_STYLES` (topology.js), keep them equal. Link labels are drawn in a layer above nodes; in the editor a labeled link has two `data-link` elements (line group + label), so count unique ids when testing. Import lays out rows: VLANs, network gear, compute, services.
